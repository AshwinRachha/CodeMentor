...
testCM("randomCollapsedRanges", function (cm) {
  ...
  cm.operation(function () {
    for (var i = 0; i < 200; i++) {
      ...
      } else cm.markText(start, Pos(start.line, start.ch + 4), {
        "className": "foo"
      });
    }
  });
...
testCM("rtlMovement", function (cm) {
  ...
  forEach(["خحج", "خحabcخحج", "abخحخحجcd", "abخde", "abخح2342خ1حج", "خ1ح2خح3حxج", "خحcd", "1خحcd", "abcdeح1ج", "خمرحبها مها!", "foobarر", "خ ة ق", "<img src=\"/בדיקה3.jpg\">", "يتم السحب في 05 فبراير 2014"], function (line) {
    ...
    for (var i = 0; i <= line.length; ++i) {
      ...
      if (i == line.length) is(cursor.offsetTop > prevY, "next line");else is(cursor.offsetLeft > prevX, "moved right");
...
testCM("getTokenTypeAt", function (cm) {
  ...
  cm.addOverlay({
    token: function (stream) {
      if (stream.match("foo")) return "foo";else stream.next();
    }
  });
