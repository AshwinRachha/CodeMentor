(function () {
  ...
  function stTest(name) {
    ...
    testCM(name, function (cm) {
      for (var i = 0; i < actions.length; i++) {
        ...
        if (typeof action == "string" && i == 0) cm.setValue(action);else if (typeof action == "string") cm.execCommand(action);else if (action instanceof Pos) cm.setCursor(action);else action(cm);
      }
    });
  }
  ...
  function argsToRanges(args) {
    ...
    var ranges = [];
    for (var i = 0; i < args.length; i += 4) ranges.push({
