...
function runTests(callback) {
  ...
  function step(i) {
    ...
    try {
      ...
      threw = true;
      if (expFail) callback("expected", test.name);else if (e instanceof Failure) callback("fail", test.name, e.message);else {
    ...
    if (!threw) {
      if (expFail) callback("fail", test.name, message || "expected failure, but passed");else callback("ok", test.name, message);
    }
    if (!quit) {
      // Run next test
      var delay = 0;
...
function parseTestFilter(s) {
  if (/_\*$/.test(s)) return new RegExp("^" + s.slice(0, s.length - 2), "i");else return new RegExp(s, "i");
}